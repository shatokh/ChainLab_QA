...
| Phase 8: Optional Testnet Readiness          | No.                                                                                                        | Documentation-only readiness. Keep the project maximally local-first; no wallet, faucet funds, RPC, or deployment required.                                                        |
...
| Phase 10: Optional Public Testnet Deployment | Yes, only if a future spec approves public testnet deployment after portfolio polish.                      | Read `docs/setup/testnet-deployment-preconditions.md`, create a separate test wallet, use free faucet funds only, use configurable public/free-tier RPC, and never commit secrets. |
